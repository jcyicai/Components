...
<script>
  ...
  export default {
    ...
    props: {
      ...
      type: {
        type: String,
        default: 'edit', // 'readonly' 只读 'edit' 编辑
      },
    },
    data() {
      return {}
    ...
    methods: {
      ...
      formatValue(val, format) {
        ...
        if (format.type === 'date') {
          ...
          // 数字
          value =
            val || val === 0 ? this.$baseNumFormat(val, format.digits || 2, format.numType || 's', format.zeroFill) : ''
        }
        return value
      },
      // 设置所有行展开
      setAllRowExpand(checked) {
        this.$nextTick(() => {
          this.$refs['xTable'].setAllRowExpand(checked)
        })
      },
      // 设置行展开
      setRowExpand(row, checked) {
        this.$nextTick(() => {
