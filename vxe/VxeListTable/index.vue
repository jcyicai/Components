<template>
  <div class="vxe-table-container">
    ...
    <!-- checkbox 列 -->
      ...
      <!--行操作栏-->
      <vxe-column v-if="showOperate" align="center" title="操作" width="160" :fixed="showExpand ? undefined : 'right'">
  ...
  export default {
    ...
    methods: {
      ...
      async getTableOption() {
        ...
        // 通过接口获取 可与后端配合
        const { data } = await getTableConfig(params)
      ...
      async handleTableOptionSave() {
        ...
        // 通过接口保存 配置
        const res = await postTableConfigSave(params).catch(() => {
